...
Check Redis memory pressure: kubectl exec -n platform deploy/redis -- redis-cli info memory
...
Check current count: kubectl exec -n platform deploy/redis -- redis-cli ZCARD drivers:available
...
Check pending entry list: kubectl exec -n platform deploy/redis -- redis-cli XPENDING ride_events dispatch_group - + 100
