...
function FiredCard({ entry }: { entry: TimelineFiredEntry }) {
  ...
  return (
    <div className="relative">
      <span className="absolute -left-6 top-0.5 flex h-[22px] w-[22px] items-center justify-center rounded-full border-2 border-critical bg-white text-critical">
      ...
      {reason && (
        <span className="mt-1 inline-block rounded-md border border-border bg-canvas px-2 py-0.5 font-mono text-[11px] text-ink-soft">
...
function ResolvedCard({ entry }: { entry: TimelineResolvedEntry }) {
  return (
    <div className="relative">
      <span className="absolute -left-6 top-0.5 flex h-[22px] w-[22px] items-center justify-center rounded-full border-2 border-healthy bg-white text-healthy">
...
export function Timeline({ entries, mode = 'full' }: { entries: TimelineEntry[]; mode?: 'latest' | 'full' }) {
  ...
  return (
    <div className="w-[190px] shrink-0 self-start rounded-[10px] border border-border bg-white p-3 sticky top-6 max-[1100px]:static max-[1100px]:w-full">
