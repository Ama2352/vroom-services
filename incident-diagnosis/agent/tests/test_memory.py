...
def test_store_knowledge_bumps_corpus_version(rdb):
    before = memory.get_corpus_version(rdb)
    memory.store_knowledge_entry(rdb, _make_knowledge(key="versioned"))
    assert memory.get_corpus_version(rdb) == before + 1


def test_successful_knowledge_update_and_delete_bump_version(rdb):
    memory.store_knowledge_entry(rdb, _make_knowledge(key="versioned"))
    before_update = memory.get_corpus_version(rdb)
    assert memory.update_knowledge_entry(rdb, "versioned", {"fix_action": "raise limit"})
    assert memory.get_corpus_version(rdb) == before_update + 1
    before_delete = memory.get_corpus_version(rdb)
    assert memory.delete_knowledge_entry(rdb, "versioned") == "deleted"
    assert memory.get_corpus_version(rdb) == before_delete + 1


def test_history_mutations_bump_corpus_version(rdb):
    memory.store_knowledge_entry(rdb, _make_knowledge(key="versioned"))
    hid = memory.store_history_entry(rdb, _make_history(knowledge_key="versioned"))
    after_store = memory.get_corpus_version(rdb)
    assert memory.update_history_entry(rdb, hid, {"symptom": "updated"})
    assert memory.get_corpus_version(rdb) == after_store + 1
    assert memory.delete_history_entry(rdb, hid)
    assert memory.get_corpus_version(rdb) == after_store + 2


def test_failed_mutation_does_not_bump_corpus_version(rdb):
    before = memory.get_corpus_version(rdb)
    assert not memory.update_knowledge_entry(rdb, "missing", {"fix_action": "x"})
    assert memory.get_corpus_version(rdb) == before
